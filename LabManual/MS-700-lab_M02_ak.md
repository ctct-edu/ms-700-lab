...
4. ゲスト アクセスをテストする
   - Teams Web クライアントから **[チーム**] を選択すると、チーム **Group_Afterwork_** が表示されます。(「チームを見つけることができません」と表示される場合は、ブラウザーの更新をお試しください。それでもチームが表示されない場合は、時間を置いてからTeams Web クライアントへの再サインイン等を試行してください。)
   
   - Group_Afterwork_チームの **[一般]** チャネルを選択し、 **[チャネルで投稿]** を選択して、次のメッセージを送信します: **Hello!**
   
   - 投稿したメッセージの **[...]** を選択します。[**削除**] オプションがないことに注意してください。
   
     (例えばAlex Wilberが、自分の投稿したメッセージの[...] を選択すると、3番目くらいに「削除」があります)
...
3. 左側のナビゲーションで、 **[秘密度ラベル**] を選択します。画面の上の方の黄色い枠の中に表示される、次の警告メッセージで [**今すぐ有効にする**] を選択し、Office Online ファイルのコンテンツ処理をアクティブ化します。
...
8. [ このポリシーの適用先を選択します ] ページで、次の設定を適用し、[ **次へ**] を選択します。
   ...
   - **Microsoft 365 Copilot (プレビュー)**: チェック ボックスの選択を解除します
...
**手記：**　**新しい DLP ポリシーが有効になるまで、1時間程度かかることがあります。手順がうまくいかない場合は、ラボを続行し、後の時点でこのタスクを実行します。**
...
7. しばらくすると、新しい会話メッセージの上に **「This message was blocked.」** という赤いテキストが表示されます。**[操作項目]** を選択します。このメッセージがブロックされた理由を確認し、**[キャンセル]**　をクリックします。
   ...
   ![image-20251123141725868](./media/image-20251123141725868.png)

...
4. 最初の秘密度ラベル - **General**を更新します。
   ...
   b.[範囲] セクションで、[アイテム] のチェックがオンであることを確認の上、[アイテム] **[アイテム]と[グループ & サイト]** の２つのチェックをオンにします。
   
   c. [ラベル付けされたアイテムの保護設定を選択する] ページで、チェック ボックスをオフのままにします。
...
​		f.[プライバシーと外部ユーザー アクセス設定を定義する] というページで
...
​		h. [スキーマ化されたデータ資産 (プレビュー)] セクションで、設定を既定のままにして、 [**次へ**] を選択します。
...
​		b.**[範囲]** セクションですべてのチェックボックスがマークされていることを確認します。
...
​			[**Microsoft Entra 条件付きアクセスを使用してラベル付きSharePoint サイトを保護する**] を選択し**、[制限された Web のみのアクセスを許可する**] を選択します。
...
​				アクセス許可を今すぐ割り当てますか、それともユーザーが決定するようにしますか?: **今すぐアクセス許可を割り当てる**
...
​			b.[**管理ユニットの割り当て**] ページでは、そのままにしておきます。
...
秘密度ラベルを作成して発行すると、ユーザーはそれらをチームに割り当てることができます。さらに、ユーザーは必要に応じて割り当てられたラベルを変更できます。このタスクでは、**Teams ロールアウト** チームに **[Internal**] ラベルを割り当てます。
...
​		Hello world！
	...
	6. 24時間後に戻ってきて確認すると、コンテンツは自動的に削除されています。
...
4. **テンプレートの利用またはカスタム ポリシーの作成** ページで、

   1. [**特定のテンプレートを検索]** 検索ボックスを選択し、「**GDPR」**と入力します。
   2. **[カテゴリ]** で [**プライバシー**] を選択し、[規制] セクションから **[GDPR の拡張**] テンプレートを選択します。
   3. [**次へ**] を選択します
...
7. [ **このポリシーの適用先を選択します** ] ページで、次の設定を適用し、[ **次へ**] を選択します。
   ...
   - **Exchange メール**: チェックボックスをオンします
   - **SharePoint サイト**: チェック ボックスをオンにします
   - **OneDrive アカウント**:チェックボックスのオンにします
...
8. [ **ポリシー設定の定義** ] ページで、テンプレートの既定の選択のまま、[ **次へ**] を選択します。

9. [ **保護対象の情報** ] ページで、既定の設定のままにして [ **次へ**] を選択します。

10. [ **保護処理** ] ページで、次の設定が構成されていることを確認し、[ **次へ**] を選択します。

   - チェックボックスがオン: **特定の量の機密情報が一度に共有されている場合に検出**

   - [**少なくとも __ 件以上の種類が同じ機密情報**] ボックスに「**1**」と入力します。

   - **[インシデントレポートのメール送信]**チェックボックスがオン

   - **[レポートに含める内容とレポートの受信者を選びます**] を選択して、右側のウィンドウを開きます

     [**連絡先の追加また削除**] を選択し、[**Joni Sherman**] のチェックボックスをオンにします。

     **[追加**] と **[保存**] を選択します

   - [**いずれかのDLP ルールが一致した場合に通知を送信する**] チェックボックスをオンにします

   - [**Microsoft 365 の場所にあるコンテンツへのアクセスを制限またはコンテンツを暗号化する]** チェック ボックスをオンにします

11. **[ アクセスと上書きの設定のカスタマイズ** ] ページで、次の設定が構成されていることを確認し、[ **次へ**] を選択します。

    - チェックボックスが選択されています: **Microsoft 365 の場所にあるコンテンツへのアクセスを制限またはコンテンツを暗号化する**
    - [**ユーザーによるメールの受信や、SharePoint、OneDrive、Teams の共有ファイル、およびPower BI アイテムへのアクセスをブロックします**] を選択します。
    - [**ヒントを見たユーザーによるポリシーの上書きを許可します]** を選択します。
    - [**組織外のユーザーのみをブロックします]** を選択します。
    - [**誤検知として報告された場合にルールを自動的に上書きします**] を選択します。

12. [**ポリシー モード**] ページで、[**ポリシーをすぐに有効にする**] を選択し、[**次へ**] を選択します。

13. [ 設定の確認 ] ページで設定を確認し、[ **送信** ] を選択してから ** [完了]** を選択します。

14. **[データ損失防止] ページ**にとどまり、ブラウザーを開いたままにします。
...
手順 11 から 13 は、**レポート** ページが廃止されたため、オプションと見なす必要があることに注意してください。

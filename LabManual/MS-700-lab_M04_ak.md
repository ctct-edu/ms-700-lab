...
グローバル (組織全体の既定) 会議ポリシーが正常変更され、会議の録画機能が無効になりました。変更がユーザーに適用されるまでには時間がかかります。
...
3. 右上にあるボタン **[今すぐ会議(Meet now)]** > [**会議を開始(Start meeting)**] を選択します。

4. **[今すぐ参加(Join now)]** を選択して会議を開始します。

   (「会議への参加を求めるユーザーを招待してください」メッセージが表示されたら、右上隅の **[X**] を選択して閉じます。)

5. 会議ウィンドウで、**[...]** More(その他) をクリックします。

6. **[Record and transcribe(レコーディングと文字起こし)]** を選択し、「Start recording(レコーディングを開始)」 のオプションがグレーアウトして選択できないことに注意してください。

8.  **[退出(Leave]** の赤いボタンを選択して会議を終了します。
...
3. [ **会議設定** ] ページで、[参加者] セクションの **[匿名ユーザーが会議に参加できる]** オプションを **オフ** にします。
4.  **[保存]**  を選択し、 **[確認]** を選択します。

テナント内のすべてのユーザーの会議設定が正常変更され、会議への匿名アクセスが無効になりました。変更がユーザーに適用されるまでには時間がかかります。
...
3. Calendar画面右上の **[+ 新規]** ドロップダウン メニューから、[**ウェビナー(Webinar)**] を選択します。
...
7. 登録サイトを公開し、リンクを共有します。
   ...
   2. **[イベント リンクのコピー]** をクリックします。
   3. 左側のナビゲーションで **[チーム(Teams)**] に戻ります。
   4. 左側のナビゲーション ウィンドウで、**Sales** チームの **[一般(General)]** チャネルを選択します。**[投稿を開始する(Start a post**] を選択し、コピーした登録リンクをテキスト ボックスに貼り付けて、**[投稿(Post)]** を選択します。
   5. サインアウトして、すべてのブラウザー ウィンドウを閉じます。

8. 会議の登録をテストします。

   1. **クライアント 2 VM** で **Alex Wilber** としてMicrosoft Teams Web クライアント( https://teams.microsoft.com/ )を参照します。
   2. **Sales** チームの **General(一般)** チャネルに移動し、投稿した登録リンクを選択します。サインインを要求されたら、Alex Wilberの資格情報を使用してください。
   ...
   5. 新しいブラウザタブを開き、 **Alex Wilberのメール(https://outlook.office.com/mail/)** を参照し、件名のメールを表示します:**IT Office Hour に登録されました**。
...
5. [**一般**] セクションで、次の設定を構成します。
   ...
   - デバイスロックPIN:**123456**
   - 言語: **英語 (米国)**
   - タイムゾーン: (**UTC-7:00) 太平洋標準時(米国およびカナダ)**
   - 日付形式: **MM/DD/YYYY**
   - 時間形式:**12時間(午前 / 午後)**

6. [**デバイスの設定**] で、次の設定を構成します。

   - スクリーンセーバーを表示する:**On、タイムアウト1分**
   - ハイコントラストを表示します:**On**
   - 勤務時間:**Set Office Hours、08:00-17:00**
   - 節電:**ON**
...
7. [**ネットワーク設定**] で、次の設定を構成します。
   ...
   - DHCPが有効:**On**
   - ログ有効:**Off**
   - デバイスの管理者パスワード:**Set Device Admin password、Pass@word1**

8. 構成プロファイルの設定が完了したら、 **[変更のレビュー]** - **[Save changes]** を選択します。
...
2. Teams Roomsの Microsoft 365 リソース アカウントを作成します。
   ...
   4. [**保存**] を選択します。
   5. **NY-TeamsRoom1の準備ができました** 画面を( **[x]** を選択して)閉じます。
...
6. **[ライセンスとアプリ]** を選択します。
7.  **[ライセンス]** で、[Microsoft Teams 国内通話プラン(米国、英国、カナダ以外のお客様)] の左のチェックボックスをオンにします。
...
#### タスク 3 – ユーザーの電話番号をオーダーする(手順のみ - 完了しないでください)
...
#### タスク 4 – ユーザーに電話番号を割り当てる(手順のみ - 完了しないでください)
...
5. [**割り当て先**] で [**Lynne Robbins**] を検索し、 [**割り当て**] を選択します。
   ...
   ※ Lynne Robbins を検索しても見つからない場合は、MOD管理者でTeams管理センターにアクセスして同じ作業を実施すると、見つかることがあります。

6. **[適用**] を選択して、電話番号をユーザーに割り当てます。

   ※様々な電話機の購入をおすすめする画面に遷移します。興味あれば参照してみてください。
...
#### タスク 2 - Joni Shermanがリソースアカウントを作成できるよう、ユーザー管理者の役割を追加
...
2. **ユーザー 管理者**ロールを **Joni Sherman** に割り当てるには
   ...
   - [**管理者ロールの管理(Manage admin roles)**] ウィンドウで、[**管理センターに対するアクセス許可(Admin center access)**] を選択します。
   - **[ユーザー 管理者(User Administrator)**] チェック ボックスをオンにし、[**変更の保存(Save Changes)**] を選択します。ウィンドウの上部に管理者の役割が更新された旨のメッセージが表示され、更新が確認されます。 **ウィンドウの右上にある [X] ボタンを選択** して、[管理者ロールの管理(Manage admin roles) ]ウィンドウを閉じます。
...
2. **Teams 管理センター**を参照します。左側のナビゲーション ウィンドウで [**音声**] を選択して展開し、 **[通話キュー]** を選択します。

3. 上部のウィンドウから **[+ 追加 - クイックセットアップ]** を選択します。

4. 次の情報を入力し、 [**次へ**] を選択します。

   - 通話キューに名前を追加する: **Contoso 通話キュー**
   - プロンプト、あいさつ、キーの言語を選択する: **日本語 (日本)** 
   - **音声アプリの使用（リソースアカウントは不要）** を選択

   次の情報を入力し、 [**送信**] を選択します。

   -  **[ユーザーとグループの追加**] を選択し、**Sales** を検索して追加。

5. 「よくできました!Contoso 通話キュー を使用する準備ができました。」画面にて、「応答メッセージと音楽を構成する」の右にある [**設定**]をクリックします。

6. 「案内応答と音楽」画面で以下内容を設定します。

   - 応答メッセージ:「応答メッセージを追加」を選択し「**Contosoへようこそ**」と入力します。
   - 保留音: **既定の音楽を再生する** が選択されていることを確認します。

7. 「**次へ**」を２回クリックします。「エージェントの選択」画面で「プレゼンスに基づくルーティング」を「**オフ**」にして 「**送信**」をクリックします。

新しい通話キューの作成には時間がかかる場合がありますが、新しいカスタム通話キューが作成されました。
...
1. **クライアント 1 VM** に接続し、提供された資格情報でサインインします。
2. 引き続き **Teams 管理センター**にいて、**Joni Sherman** (JoniS@YourTenant.onmicrosoft.com) としてサインインする必要があります。
3. 左側のナビゲーション ウィンドウで、 **[音声**] を選択し、 [**リソースアカウント**] を選択します。
4. [**リソース アカウント**] ページで、 [**+追加**] を選択します。
5. 「**リソースアカウントを追加**」パネルで、以下の情報を入力して選択してください:
   - 表示名:**Contoso AutoAttendant**
   - ユーザー名:**pilot_autoattendant1**
   - ドメインドロップダウンでテナントドメインを選択
   - リソースアカウントの種類 : **自動応答**
6. 「**保存**」を選択します。右側のウィンドウで、「**こちらをクリックしてください**」リンクをクリックし、[**リソースアカウントの追加**] をクリックし、「リソース アカウントの追加」ウィンドウでドメイン名のみを設定して「保存」をクリックします。
7. 「リソースアカウントの追加」画面で表示名に「Contoso 自動応答」、一意のユーザー名を　**pilot_autoattendant1** @ **YourTenant.onmicrosoft.com** となるようにして、「保存」「追加」とクリックします。
8. Microsoft 365 管理センター( https://admin.microsoft.com  )にアクセスします、[ユーザー] を選択し、[**アクティブなユーザー**] を選択します。
9. Contoso AutoAttendant アカウントを選択し、[**ライセンスとアプリ**] タブを選択します。
10. [NY-TeamsRoom1@YourTenant.onmicrosoft.com ページの [**ライセンスとアプリ**] タブで、[**Microsoft Teams 電話リソースアカウント**] を選択し、[**変更の保存**] を選択します。
11.  **Teams 管理センター** に戻ります。左側のナビゲーション ウィンドウで、 **[音声**] を選択し、 [**自動応答**] を選択します。
12. 上部のウィンドウから **[+ 追加**] を選択して、新しい自動応答を作成します。
13. 画面下の方の「**高度なセットアップ**」リンクをクリックします。
14. 次の情報を入力します。
    - 表示名: **Contoso 自動応答**
    - 発信者がこの自動応答に到達する方法：リソースアカウントを通じて
      - [リソースアカウントの割り当て] をクリックし、Contosoと入力して「**Contoso AutoAttendant**」の **[追加]** を選択して、その後に下の方の **[保存]** を選択します。 
    - オペレーター ‎(‎省略可能)‎‎: **「音声アプリ」** を選択し、右枠にContosoと入力して「**Contoso 通話キュー**」を選択
    - タイム ゾーン: (**UTC+09:00) 大阪、札幌、東京**
    - 言語: **日本語 (日本)** 
    - 音声入力：**オフ**
15. [**次へ**] を選択します。
16. **[通話処理時間]** ページで、以下を構成します。
    - 「タイム ゾーン」で **[ (UYC+9:00) 大阪、札幌、東京]** を選択します 
    - 「勤務時間および勤務時間外」を **[毎日同じ時間を使用する]** に変更します。
    - **月曜日**から**金曜日の** **9:00 から17:00 まで** の勤務時間を構成する
    - **土曜日**から**日曜日**はチェックボックスをオフにし、空白にします。
17. [**次へ**] を選択します。
18. [勤務時間内通話フロー] ページで、次のように構成します。
    - あいさつのオプション:「応答メッセージを追加」を選択し「**いらっしゃいませ。あなたが電話した相手は現在休暇中のため、あなたの電話はオペレーターにリダイレクトされます。**」と入力します。
    - 通話ルーティング オプション:  **「通話をリダイレクト」** を選択し、**「音声アプリ」** を選択し、右枠にContosoと入力して「**Contoso 通話キュー**」を選択します。
19. [**次へ**] を選択します。
20. **[営業時間外の通話フロー]** ページで、以下を構成します。
    - 通話ルーティング オプション: **切断**
21. [**次へ**] を選択します。
22. **祝日の通話フロー** ページで、[**次へ**] を選択します。
23. [**ダイヤル の範囲**] ページで、[**次へ**] を選択します。
24. [**送信**] を選択して、自動応答の作成を完了します。

これで、自動応答のリソース アカウントが作成され、自動応答が作成されました。



（オプション）Joni Shareman の資格情報で teams.microsoft.com にアクセスし、「通話」メニューから Contoso 等入力して「Contoso AutoAttendant」を検索し、通話してみてください。時間によって異なる応答メッセージを聞くことができるかと思います。

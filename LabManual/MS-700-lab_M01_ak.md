...
5. 上部のリボンで [**ポリシーの削除**] を選択し **> [はい]** を選択します。
  ...
  異なるユーザーのグループに対して多数のポリシーを個別に管理することによる管理オーバーヘッドを回避するには、ポリシー パッケージを使用してポリシーを論理ユニットにグループ化することを評価する必要があります。このタスクでは、既定のポリシー パッケージを確認し、第一線の作業者の既定のポリシー パッケージを変更する必要があります。
    ...
    14. 一覧から **フロントライン ワーカー (既定)** を再度選択します。

    15. **[通話ポリシー**] の横にある **[Frontline_worker**] を選択します。

    16. [ **有料料金のバイパスを防止してPSTN 経由で通話を送信**] をオンにします。

    17. **[通話中は取り込み中]**  を [**オン**] にします。

    18. [**保存**] を選択します。 

    19. [**確認**] を選択します。

    20. 左側のナビゲーション ウィンドウから **[ポリシー パッケージ**] に移動します。

    21. **フロントライン ワーカー (既定)** ポリシー パッケージがオンになっていることを確認します。

    22. 上部のメニューから [**ユーザーを管理**] を選択します。

    23. 検索ボックスに「**Allan**」と入力し、**[Allan Deyoung]** から [**追加**] を選択して **[適用**] を選択します。

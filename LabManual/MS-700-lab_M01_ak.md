...
3. **Alex Wilber の**ライセンスを確認する
   - 同じタブの **[ライセンス(Licenses)**] セクションで、 **Microsoft 365 E5** と **Microsoft Teams Enterprise** が選択されていることを確認します。
...
3. Teams 管理Joni センターの左側のナビゲーションで、[**Teams**] > **[チームを管理**] を選択します。作成すると、組織内のチームが表示されます。

4. Teams 管理センターの左側のナビゲーションで、[**Teams**] >  **[Teams ポリシー**] を選択します。**Global (組織全体の既定)** という名前の既定の Teams ポリシーを確認できます。
...
3. **Microsoft Teams PowerShell モジュール**をインストールする
   ...
   - PowerShell ウィンドウで、次のコマンドレットを入力し、**Enter キーを押します。**
     ...
     **「Y**」と入力し、**Enter** キーを 押して、NuGet プロバイダーと信頼されていないリポジトリのインストールを確認します。
...
6. **Microsoft Graph モジュー ル** 等のインストール
...
5. [**グループを作成**] を選択します。
6. [**閉じる**] を選択します。
7. しばらく待ってから、グループが表示されるまで **[最新の情報に更新**] を選択します。「Teams Rollout」の **[Teams の状態]** 列に Teams アイコンがあり、有効であることを確認します。
...
### 演習 4: Microsoft Teams のライフサイクル管理とガバナンスを実装する
  ...
  1. **クライアント 1 VM** に接続し、**MOD 管理者**として Entra ID 管理センター (https://entra.microsoft.com/) にアクセスします。多要素認証が要求されますので、次の手順を参考に設定します。

  2. 「アカウントをセキュリティで保護しましょう」画面が表示されます。「次へ」をクリックします。

     「Microsoft Authenticator のインストール」画面で「次へ」をクリックします。

3. 「アプリでアカウントをセットアップする」画面で「次へ)」をクリックします。

4. 「QR コードをスキャンします」画面が表示されます。お手持ちのスマートフォンのMicrosoft Authenticatorアプリにて、「+」(画面右上にあります) - 「職場または学校のアカウント」 - 「QRコードをスキャン」と選択して、QRコードをスキャンします。 InPrivate ブラウザー ウィンドウに戻り「次へ」をクリックします。!

5. 「試してみましょう」画面で表示された番号を使用して、Authenticatorアプリの認証を実施します。

6. 「Authenticator Added」画面で「完了」をクリックします。

7. 「サインインの状態を維持しますか?」と表示される場合があります。「いいえ」を選択してください。

8. Entra ID 管理センターの画面が表示されます。もし英語表示だった場合は、次の手順で日本語表記に変更してください。

   画面右上の歯車のマーク「Settings」をクリックした後、左の「Language + region」をクリックします。Language と Regional format でいずれも日本語を選択し、[Apply] - [OK] をクリックして変更します。

  9. 左側のナビゲーション ウィンドウで、[**グループ**] > [**すべてのグループ**] を選択します。

  10. グループ|すべてのグループ ページの左のメニューから、[**有効期限**] を選択します。

  11. グループ|有効期限 ページで、次の設定を構成します。
  ...
  2. Alex Wilber のロケーションをカナダに更新
     ...
     - 左側のナビゲーション ウィンドウで、 **[ユーザー]** > **[すべてのユーザー**] を選択します。
  ...
  3. 左側のナビゲーション ウィンドウで、**[グループ**] > **[すべてのグループ**] を選択します。
